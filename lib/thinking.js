...
/**
 * Assistant parts as ordered reasoning/text segments, ready to render in place.
 *
 * Octavus opens a fresh `reasoning` part per thought and streams deltas into it,
 * so part boundaries are genuine thought boundaries and are preserved. Runs of
 * consecutive text parts are merged because together they form one Markdown
 * document — rendering them separately would break constructs that span parts,
 * such as a list. Segments with nothing to show are dropped, except a reasoning
 * part that is still streaming, which carries the "Thinking…" state.
 *
