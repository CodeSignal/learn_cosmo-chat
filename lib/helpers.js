...
// (case-insensitively) and returns its `strings` map. Returns `{}` when no
// locale matches or the language is blank. `locales` is an array of parsed
// i18n files: [{ languageNames: string[], strings: object }].
...
// Object-map keys that merge per-entry instead of replacing the whole object.
// Everything else (scalars, arrays, other objects) is replaced by the override.
const CONFIG_MAP_KEYS = new Set(['strings', 'modelDisplayNames']);

// Layers practice overrides on top of the base chat-config. Used so a shared
// base setup can ship with the project while individual practices pin only the
// settings they need via chat-config.override.json.
