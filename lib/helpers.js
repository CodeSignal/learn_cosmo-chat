...
// A `null` verbosity injects nothing, leaving the prompt exactly as-is.
...
const DEFAULT_VERBOSITY = 'detailed';

// Resolves the effective verbosity: options override config, an absent value
// falls back to the default, and an explicit `null` disables verbosity control.
export function resolveVerbosity(options = {}, config = {}) {
  if (options.verbosity !== undefined) return options.verbosity;
  if (config.verbosity !== undefined) return config.verbosity;
...
export function buildSessionInput(options = {}, config = {}) {
  ...
  if (verbosity !== null && VERBOSITY_LEVELS[verbosity]) {
