...
export function buildSessionInput(options = {}, config = {}) {
  ...
  // Spoken language for responses. Options override config; when neither
  // provides one we default to English so the prompt always has a value.
  input.LANGUAGE = options.language ?? config.language ?? 'English';
